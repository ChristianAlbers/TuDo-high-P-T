...
class spectrum:
	def __init__(self,x=[],y=[],error=ar([0]),background=ar([0])):
		...
		if background.any() == False:
			self.background = ar(range(len(y)))
		else:
	...
	def __add__(self,new):
		...
		if (isinstance(new,spectrum) == True):
			if len(new.x) == 0:
				...
				return new#
			new_x=new.x
			return spectrum(new_x, np.interp(new_x, self.x, self.y, 0, 0) + np.interp(new_x, new.x, new.y, 0, 0),np.sqrt(np.interp(new_x, self.x, self.error, 0, 0)**2 + np.interp(new_x, new.x, new.error, 0, 0)**2), np.interp(new_x, self.x, self.background, 0, 0) + np.interp(new_x, new.x, new.background, 0, 0))
		elif (type(new) == float or type(new) == int or type(new) == np.float64 or type(new) == np.int or type(new) == np.int64):
			return spectrum(self.x,self.y + new, self.error)
	...
	def n(self, min = 0, max = 0):
		"""normalize to area"""
		return spectrum(self.x,self.y/self.ave(min, max),self.error/self.ave(min, max))
