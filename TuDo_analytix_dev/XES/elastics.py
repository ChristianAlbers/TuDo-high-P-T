...
class XES_elastics:
	...
	def load_images(self,
		...
		if not any(self.images):
			...
			self.mask[:]=1
		for file in images:
			self.images[file]=fabio.open(os.path.normpath(file))
			...
			for i, image in enumerate(self.images):
				...
				if show:
					if i==0:
						...
						show_img+=data
				if mode == "auto":
					rois=self.auto_rois(img=self.images[image].data,roi_number=self.crys_number)
					box_rois[i]=rois
					...
					if type(custom_rois) in [list, np.array, np.ndarray]:
						...
						print("choose mode auto or give custom_rois as [[y0,y1,x0,x1],[y0,y1,x0,x1],...]")
						return None
				roiscount=0
				for roi in rois:
	...
	def peakfinder(self,
		...
		return peaks
	'''
	def auto_rois(self, img, roi_number, show=False):
		...
		for i, peak in enumerate(y_peaks):
			...
			out_rois[i,0]=int(peak-5) 	#y0
			out_rois[i,1]=int(peak+5)	#y1

			line_img=img[int(peak-5):int(peak+5),:]
			...
			out_rois[i,2]=int(x_peaks[0]-5) 	#x0
			out_rois[i,3]=int(x_peaks[0]+5)	#x1
	...
	def show_images(self):
		for im in self.images:
			...
			plt.imshow(self.images[im].data,vmax=self.images[im].data.max()/5)