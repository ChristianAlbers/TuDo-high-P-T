...
class spectrum:
  def __init__(self,x=[],y=[],error=ar([0])):
  ...
  def __and__(self,new):
    ...
    if (isinstance(new,spectrum) == True):
      ...
      f = InterpolatedUnivariateSpline(self.x, self.y, k=3)
      f2 = InterpolatedUnivariateSpline(new.x, new.y, k=3)
      return spectrum(new_x, f(new_x) + f2(new_x))
    elif (type(new) == float or type(new) == int or type(new) == np.float64 or type(new) == np.int or type(new) == np.int64):
      return spectrum(self.x,self.y + new, self.error)
  def __call__(self, min = 0, max = 0):
    """defines the () function to cut the spectrum"""
    #extend to add values on side?
    if min == max:
      return spectrum(self.x,self.y,self.error)
    else:
       return spectrum(self.x[np.searchsorted(self.x, min, side="left"):np.searchsorted(self.x, max, side="right")],self.y[np.searchsorted(self.x, min, side="left"):np.searchsorted(self.x, max, side="right")],self.error[np.searchsorted(self.x, min, side="left"):np.searchsorted(self.x, max, side="right")])
  def __repr__(self):
  ...
  def __add__(self,new):
    ...
    if (isinstance(new,spectrum) == True):
      if len(new.x) == 0:
        ...
        return new
      new_x = ar(list(k for k, v in itertools.groupby(heapq.merge(self.x, new.x))))
      return spectrum(new_x, np.interp(new_x, self.x, self.y, 0, 0) + np.interp(new_x, new.x, new.y, 0, 0))
    elif (type(new) == float or type(new) == int or type(new) == np.float64 or type(new) == np.int or type(new) == np.int64):
      return spectrum(self.x,self.y + new, self.error)
  def __sub__(self, new):
    """defines - operator for subtraction"""
    if (isinstance(new,spectrum) == True):
      new = spectrum(new.x,-1*new.y,new.error)
      return self + new
    elif (type(new) == float or type(new) == int or type(new) == np.float64 or type(new) == np.int or type(new) == np.int64):
      return spectrum(self.x,self.y - new, self.error)
  def __mul__(self, new):
    ...
    if (isinstance(new,spectrum) == True):
      ...
      return spectrum(self.x, self.y * new, self.error * new)
  ...
  def __lshift__(self, new):
    ...
    return spectrum(self.x - new,self.y)
  ...
  def sort(self):
    ...
    return spectrum(self.x[order],self.y[order])
  def p(self, newfigure=0, newlabel = "", style = "", ng = 0):
    """plots the spectrum"""
    if newfigure == 1:
      plt.figure()
    plt.plot(self.x,self.y, style ,label = newlabel)
    plt.show()
    if ng == 1:
      plt.legend()
      plt.grid()
  ...
  def s(self):
    ...
    return spectrum(self.y,self.x,self.error)
  ...
  def n(self, min = 0, max = 0):
    """normalize to area"""
    return spectrum(self.x,self.y/self.ave(min, max))
  def rebin(self,rebin_val): 
    ...
    if (type(rebin_val) == np.ndarray):
      ...
      return spectrum(new_x, np.interp(new_x, self.x, self.y, 0, 0))
  def reverse(self):
    """see r()"""
    return spectrum(self.x[::-1],self.y[::-1])
  def r(self):
  ...
  def save(self,savefilename,force=False):
    ...
    if os.path.exists(savefilename) == True and force == False:
      overwritefile = raw_input("output file exists, overwrite? (y/n)")
      if overwritefile != "y" and overwritefile != "Y":
        print("aborted")
        return
    specfile = open(savefilename,"w")
    for i in range(len(self.x)):
      specfile.write(str(self.x[i]) + " " + str(self.y[i]))
      specfile.write("\n")
    specfile.close()
  def b(self, broaden):
